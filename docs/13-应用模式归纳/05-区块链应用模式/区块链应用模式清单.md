...
- [区块链应用模式清单 / Blockchain Application Pattern Checklist](#区块链应用模式清单--blockchain-application-pattern-checklist)
  ...
  - [📚 **四、典型案例 / Part 4: Typical Cases**](#-四典型案例--part-4-typical-cases)
    ...
    - [案例4：NFT交易网络分析](#案例4nft交易网络分析)
    - [案例5：跨链桥安全验证](#案例5跨链桥安全验证)
...
ConsistencyProperty ==
    \A s \in ReachableStates :
        \A n1, n2 \in HonestNodes :
            ...
            => Value(n1, s) = Value(n2, s)  // 一致性

ByzantineFaultTolerance ==
    \A s \in ReachableStates :
        /\ Cardinality(ByzantineNodes) < Cardinality(Nodes) / 3
        => ConsistencyProperty  // 拜占庭容错
```
...
```python
# Graph Transformer: 区块链交易异常检测（2025最新方法）
import torch
import torch.nn as nn

class BlockchainAnomalyDetectorGT(nn.Module):
    """基于Graph Transformer的区块链交易异常检测器"""
    ...
    def forward(self, transaction_graph, address_features):
        """
        前向传播
        transaction_graph: NetworkX图，包含地址节点和交易边
        address_features: 地址特征（余额、交易次数、gas使用等）
        """
        # 节点特征编码
...
```python
# Petri Graph Neural Networks: 智能合约安全分析（2025最新方法）
import torch
import torch.nn as nn
import networkx as nx
...
class SmartContractSecurityAnalyzerPGNN(nn.Module):
    ...
    def __init__(self, num_states, num_operations, hidden_dim=128):
        ...
        self.vulnerability_head = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(hidden_dim // 2, 5),  # 5种常见漏洞类型
            nn.Sigmoid()
        )
    ...
    def forward(self, contract_petri_net, state_features):
        """
        前向传播
        contract_petri_net: NetworkX图，包含state（库所）和operation（变迁）节点
        state_features: 状态特征（余额、权限、锁定状态等）
        """
        # 初始化嵌入
        embeddings = {}
        ...
        if state_embeddings:
            ...
            return vulnerability_scores  # [重入攻击, 整数溢出, 权限控制, 拒绝服务, 其他]
...
**工具组合**: NetworkX + Neo4j + GUDHI + Dune Analytics + Etherscan
...
**工具组合**: CPN Tools + TLA+ + NetworkX + Chainlink

**关键代码示例**:

```cpn
// CPN Tools: 跨链桥Petri网模型
        ...
        for chain in self.graph.nodes():
            if self.graph.nodes[chain].get('type') == 'chain':
                ...
                conservation_status[chain] = {
                    'locked': locked,
                    'minted': minted,
                    'conserved': abs(locked - minted) < 0.01  # 允许小的浮点误差
                }
...
- [x] ✅ 补充更多案例（NFT分析、跨链桥安全）
- [ ] 添加具体代码示例
- [x] DeFi协议安全分析深入指南（见 [03-工具集成指南.md](03-工具集成指南.md) 第四部分）
...
- **Ivy**: 专门用于分布式协议验证
- **Model Checking Contest**: 包含PBFT等协议的标准模型
...
- **研究**: 使用Petri网验证DeFi协议的安全性
- **应用**: Uniswap、Compound等协议验证
- **工具**: CPN Tools + TLA+组合
