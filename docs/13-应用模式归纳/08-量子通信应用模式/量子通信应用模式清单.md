...
### 案例6：量子网络性能优化（Graph Transformer）

**场景**: 使用Graph Transformer优化量子网络拓扑和路由性能

**建模选择**: Graph Transformer

**实现方案**:

```text
步骤1: 量子网络图构建
    - 节点: 量子节点（带特征：保真度、容量、延迟）
    - 边: 量子链路（带特征：距离、损耗、噪声）
    - 图结构: 量子网络拓扑

步骤2: Graph Transformer建模
    - 节点特征编码
    - 全局注意力机制学习节点关系
    - 多尺度量子网络分析
...
**工具组合**: PyTorch + PyTorch Geometric + NetworkX + Qiskit
...
class QuantumNetworkGraphTransformer(nn.Module):
    """基于Graph Transformer的量子网络性能优化器"""
    ...
    def __init__(self, input_dim, hidden_dim=128, num_layers=3, num_heads=8):
        ...
        self.performance_head = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(hidden_dim // 2, 3)  # [fidelity, throughput, latency]
        )
    ...
    def optimize_routing(self, x, edge_index, source, target):
        """优化路由路径"""
        ...
        routing_score = self.routing_head(pair_feat)

        return routing_score

    def predict_performance(self, x, edge_index):
        """预测网络性能"""
        node_embeddings = self.forward(x, edge_index)
        performance = self.performance_head(node_embeddings)

        return {
            'fidelity': performance[:, 0],
...
**场景**: 使用PGNN建模量子网络中的多模态信息流和纠缠分发

**建模选择**: Petri Graph Neural Networks (PGNN)
...
class QuantumNetworkFlowPGNN(nn.Module):
    """基于PGNN的量子网络流建模器"""
    ...
    def forward(self, quantum_petri_net, node_features, operation_features):
        """
        前向传播
        quantum_petri_net: NetworkX图，包含node（库所）和operation（变迁）节点
        node_features: 节点特征（保真度、容量等）
        operation_features: 操作特征（操作类型、参数等）
        """
        # 初始化嵌入
        embeddings = {}
        ...
        for layer in self.propagation_layers:
            ...
            for node in quantum_petri_net.nodes():
                # 聚合邻居信息（遵循Petri网流约束）
                neighbor_embeddings = []
                for neighbor in quantum_petri_net.neighbors(node):
    ...
    def predict_entanglement_distribution(self, quantum_petri_net, node_features,
                                          operation_features, source_node, target_node):
        """预测纠缠分发"""
        embeddings = self.forward(quantum_petri_net, node_features, operation_features)

        source_emb = embeddings[source_node]
        target_emb = embeddings[target_node]

        pair_feat = torch.cat([source_emb, target_emb], dim=-1)
        entanglement_prob = self.entanglement_head(pair_feat)

        return entanglement_prob

    def predict_fidelity(self, quantum_petri_net, node_features, operation_features, path):
        """预测路径保真度"""
        embeddings = self.forward(quantum_petri_net, node_features, operation_features)
...
- [x] 与实际量子通信工具集成指南（见 [03-工具集成指南.md](03-工具集成指南.md)）
