...
### 案例6：蛋白质结构预测（Graph Transformer）

**场景**: 使用Graph Transformer预测蛋白质结构和功能
...
```text
步骤1: 蛋白质图构建
    - 节点: 氨基酸残基（带特征：类型、位置、理化性质）
    - 边: 残基相互作用（带特征：距离、角度、相互作用类型）
    - 图结构: 蛋白质3D结构
...
**工具组合**: PyTorch + PyTorch Geometric + AlphaFold2 + ESM
...
class ProteinStructureGraphTransformer(nn.Module):
    """基于Graph Transformer的蛋白质结构预测器"""
    ...
    def predict_structure(self, x, edge_index):
        """预测蛋白质结构"""
        node_embeddings = self.forward(x, edge_index)

        # 二级结构预测
        ss_pred = self.ss_head(node_embeddings)

        # 三级结构预测
        coords = self.coord_head(node_embeddings)

        # 功能位点预测
        function_sites = self.function_head(node_embeddings)
...
### 案例7：基因调控网络多模态分析（PGNN）

**场景**: 使用PGNN分析多组学数据（转录组、表观组、蛋白组）构建的基因调控网络
...
**工具组合**: PyTorch + NetworkX + GINtoSPN + pySCENIC
...
class MultiOmicsRegulatoryNetworkPGNN(nn.Module):
    """基于PGNN的多组学基因调控网络分析器"""
    ...
    def forward(self, regulatory_petri_net, gene_features, tf_features,
                epigenetic_features, protein_features):
        """
        前向传播
        regulatory_petri_net: NetworkX图，包含gene、tf、epigenetic、protein节点
        """
        # 初始化嵌入
        embeddings = {}
        ...
        for layer in self.propagation_layers:
            ...
            for node in regulatory_petri_net.nodes():
                # 聚合邻居信息（遵循Petri网流约束）
                neighbor_embeddings = []
                for neighbor in regulatory_petri_net.neighbors(node):
    ...
    def predict_regulation(self, regulatory_petri_net, gene_features, tf_features,
        ...
        """预测调控关系"""
        embeddings = self.forward(regulatory_petri_net, gene_features, tf_features,
                                 epigenetic_features, protein_features)
        ...
        regulation_strength = self.strength_head(pair_feat)

        return {
            'regulation_probability': regulation_prob,
...
- [x] 与实际生物信息学工具集成指南（见 [03-工具集成指南.md](03-工具集成指南.md)）
