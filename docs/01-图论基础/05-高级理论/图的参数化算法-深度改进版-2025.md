...
使用树分解和动态规划。对树宽为 $k$ 的图，可以构造树宽为 $k$ 的树分解，然后在树分解上进行动态规划。

状态空间大小为 $O(2^k)$ 或 $O(3^k)$（取决于问题），状态转移需要 $O(k)$ 时间，因此总复杂度为 $O(f(k) \cdot n)$，其中 $f(k) = 2^k$ 或 $3^k$。

对于固定 $k$，$f(k)$ 是常数，因此算法是线性时间的。□
...
如果问题 $(I, k)$ 是FPT的，则存在算法在 $O(f(k) \cdot |I|^{O(1)})$ 时间内解决。

如果 $|I| > f(k)$，则算法可以在 $O(f(k) \cdot |I|^{O(1)})$ 时间内解决，因此可以构造核大小为 $g(k) = f(k)$ 的核化算法。

如果 $|I| \leq f(k)$，则实例本身就是核。
