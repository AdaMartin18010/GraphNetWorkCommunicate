...
设树边 $e$ 将 $T$ 分成两个连通分量 $C_1$ 和 $C_2$。设 $E_1 = \sigma^{-1}(C_1)$ 和 $E_2 = \sigma^{-1}(C_2)$ 是对应的边子集。

对于顶点 $v \in \lambda(e)$，存在边 $e_1 \in E_1$ 和 $e_2 \in E_2$ 使得 $v \in e_1 \cap e_2$。这意味着 $v$ 是连接 $E_1$ 和 $E_2$ 的顶点。

因此，$\lambda(e)$ 确实是连接两个分量的边的端点集合。□
