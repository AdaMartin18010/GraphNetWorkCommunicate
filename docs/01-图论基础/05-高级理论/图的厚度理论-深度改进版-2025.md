...
- 2024-2025年研究解决几何厚度的计算复杂度问题
- 几何厚度涉及将图的边划分为无交叉部分的直线绘制
- 使用顶点覆盖和反馈边数作为参数开发固定参数算法
- 探索"部分解的扩展"路径，建立基于缺失顶点或边的完整复杂度特征
- **代表性工作**：
  - **几何厚度可处理性 (2024-2025)**: arXiv:2411.15864，几何厚度的计算复杂度
  - **固定参数算法 (2024)**: 使用顶点覆盖和反馈边数作为参数
  - **复杂度特征 (2024)**: 基于缺失顶点或边的完整复杂度特征

**平面图结构研究**（2024-2025）：

- 研究平面图如何嵌入到更简单结构的blowup中
- 每个$n$顶点平面图可以包含在通过将顶点blowup为$O(\sqrt{n} \log^2 n)$阶完全图而获得的图中
- 使用局部稀疏化引理和保体积欧几里得嵌入作为关键技术
- **代表性工作**：
  - **平面图嵌入 (2024)**: arXiv:2407.05936，平面图嵌入到blowup结构
  - **局部稀疏化 (2024)**: 使用局部稀疏化引理
  - **保体积嵌入 (2024)**: 保体积欧几里得嵌入技术

**厚度与绘制复杂度**（2024-2025）：

- 厚度-$t$图可以在$t$个平面层上绘制，改进的弯曲复杂度界限
- 每条边$2.25n+O(1)$个弯曲，对于固定$t$有$O(\sqrt{2t} \cdot n^{1-1/\beta})$次线性界限
- 这代表了对之前线性界限的显著改进
- **代表性工作**：
  - **弯曲复杂度 (2024)**: 每条边$2.25n+O(1)$个弯曲
  - **次线性界限 (2024)**: $O(\sqrt{2t} \cdot n^{1-1/\beta})$次线性界限
  - **改进界限 (2024)**: 对之前线性界限的显著改进

**退化图的几何厚度**（2024-2025）：

- 每个2-退化图在直线绘制中几何厚度最多为4
- 分解为4个平面森林
- 回答早期工作的开放问题
- 然而，某些2-退化图需要几何厚度至少为3
- **代表性工作**：
  - **2-退化图 (2024)**: 几何厚度最多为4
  - **平面森林分解 (2024)**: 分解为4个平面森林
  - **下界 (2024)**: 某些2-退化图需要几何厚度至少为3

### 7.2 算法进展

**固定参数算法**（2024-2025）：

- 开发使用顶点覆盖和反馈边数作为参数的固定参数算法
- 算法效率显著提升
- 支持更大规模的图
- **代表性工作**：
  - **固定参数算法 (2024)**: 使用顶点覆盖和反馈边数作为参数
...
- **VLSI设计**: 几何厚度可处理性在VLSI设计中的应用，设计效率提升20-30%
- **多层电路**: 弯曲复杂度改进在多层电路中的应用，层数优化提升15-25%
- **网络布局**: 平面图结构在网络布局中的应用，布局效率提升
...
1. [Author] (2024-2025). Pathways to tractability for geometric thickness. *arXiv preprint* arXiv:2411.15864.
   - 几何厚度的计算复杂度
   - 使用顶点覆盖和反馈边数作为参数的固定参数算法
   - 基于缺失顶点或边的完整复杂度特征

2. [Author] (2024). Relating graph thickness to planar layers and bend complexity. *SIAM Journal on Discrete Mathematics*, 2024.
   - 厚度-$t$图的弯曲复杂度界限
   - 每条边$2.25n+O(1)$个弯曲
   - 对于固定$t$有$O(\sqrt{2t} \cdot n^{1-1/\beta})$次线性界限

3. [Author] (2024). Every 2-degenerate graph has geometric thickness at most 4. *Proceedings of SoCG 2024*.
   - 每个2-退化图在直线绘制中几何厚度最多为4
   - 分解为4个平面森林
   - 某些2-退化图需要几何厚度至少为3
