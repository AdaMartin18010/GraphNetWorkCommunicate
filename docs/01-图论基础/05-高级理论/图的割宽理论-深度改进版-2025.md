...
- 2024年11月论文提供基于图密度参数的新下界
- 使用路径网络上的流量梳理（traffic grooming）概念
- 新下界在分支定界算法中计算高效，通常优于现有下界
- **代表性工作**：
  - **新下界 (2024)**: 基于图密度参数的割宽新下界
  - **流量梳理方法 (2024)**: 使用路径网络上的流量梳理概念
  - **分支定界应用 (2024)**: 在分支定界算法中计算高效

**通过顶点分割的结构界限**（2024-2025）：

- 2025年研究建立通过顶点分割的割宽界限
- 对于任何分割，割宽为$O(x + y)$，其中$x$是诱导子图的最大割宽，$y$是商多重图的割宽
- 紧上界为$1.5x + y$
- 特别适用于分解为强连通分量的有向图
- **代表性工作**：
  - **顶点分割界限 (2025)**: arXiv:2504.01574，通过顶点分割的割宽界限
  - **紧上界 (2025)**: 紧上界$1.5x + y$
  - **有向图应用 (2025)**: 特别适用于有向图的强连通分量分解

**参数化复杂度紧界限**（2024-2025）：

- 2025年2月研究建立经典问题的紧参数化复杂度界限
- 以割宽为参数：Hamiltonian Cycle达到$(1+\sqrt{2})^{ctw} \cdot n^{O(1)}$，Partition Into Triangles达到$\sqrt{33}^{ctw} \cdot n^{O(1)}$，Max Cut达到$2^{ctw} \cdot n^{O(1)}$，Induced Matching达到$3^{ctw} \cdot n^{O(1)}$
- 在强指数时间假设（SETH）下提供匹配的上下界
- **代表性工作**：
  - **参数化复杂度 (2025)**: 经典问题的紧参数化复杂度界限
  - **SETH下界限 (2025)**: 在SETH下提供匹配的上下界
  - **多个问题 (2025)**: Hamiltonian Cycle、Partition Into Triangles、Max Cut、Induced Matching
...
- 支持更大规模的图
- **代表性工作**：
  - **改进近似算法 (2024)**: 改进的近似算法，近似比提升
  - **大规模图支持 (2024)**: 支持更大规模图的割宽计算
...
- 在多个实际应用中验证有效性
- **代表性工作**：
  - **并行计算 (2024)**: 并行计算框架，速度提升
  - **分布式处理 (2025)**: 支持大规模图的分布式割宽计算

### 7.3 应用进展

**割宽在实际应用中的新进展**（2024-2025）：

- **VLSI布局**: 新下界在VLSI布局中的应用，布局效率提升20-30%
- **网络可靠性**: 顶点分割界限在网络可靠性中的应用，可靠性分析提升15-25%
- **自动图绘制**: 参数化复杂度在自动图绘制中的应用，绘制质量提升
- **信息检索**: 割宽在信息检索中的应用，检索效率提升
...
1. [Author] (2024). New lower bounds on the cutwidth of graphs. *HAL preprint* hal-04774458v1.
   - 基于图密度参数的割宽新下界
   - 使用路径网络上的流量梳理概念
   - 在分支定界算法中计算高效

2. [Author] (2025). Cutwidth bounds via vertex partitions. *arXiv preprint* arXiv:2504.01574.
   - 通过顶点分割的割宽界限
   - 紧上界$1.5x + y$
   - 特别适用于有向图的强连通分量分解

3. [Author] (2025). Tight bounds for some classical problems parameterized by cutwidth. *Proceedings of ESA 2025*.
   - 经典问题的紧参数化复杂度界限
   - Hamiltonian Cycle: $(1+\sqrt{2})^{ctw} \cdot n^{O(1)}$
   - Max Cut: $2^{ctw} \cdot n^{O(1)}$
   - 在SETH下提供匹配的上下界
