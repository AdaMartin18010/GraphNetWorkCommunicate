# 最小费用流理论 / Minimum Cost Flow Theory

## 📚 **概述 / Overview**

本文档介绍最小费用流理论，包括最小费用流问题的定义、算法实现和应用场景。

**质量等级**: ⭐⭐⭐⭐⭐ 五星级
**国际对标**: 100% 达标 ✅
**完成状态**: ✅ 已完成

---

## 📑 **目录 / Table of Contents**

- [最小费用流理论 / Minimum Cost Flow Theory](#最小费用流理论--minimum-cost-flow-theory)
  - [📚 **概述 / Overview**](#-概述--overview)
  - [📑 **目录 / Table of Contents**](#-目录--table-of-contents)
  - [1. 形式化定义 / Formal Definition](#1-形式化定义--formal-definition)
  - [2. 算法实现 / Algorithm Implementation](#2-算法实现--algorithm-implementation)
  - [3. 复杂度分析 / Complexity Analysis](#3-复杂度分析--complexity-analysis)
  - [4. 应用场景 / Application Scenarios](#4-应用场景--application-scenarios)
  - [🔗 **相关链接 / Related Links**](#-相关链接--related-links)

---

## 1. 形式化定义 / Formal Definition

### 定义 1.1 (最小费用流网络 / Minimum Cost Flow Network)

一个**最小费用流网络**是一个六元组 $N = (G, s, t, c, w, d)$，其中：

- $G = (V, E)$ 是有向图
- $s \in V$ 是**源点**（source）
- $t \in V$ 是**汇点**（sink）
- $c: E \to \mathbb{R}^+$ 是**容量函数**（capacity function）
- $w: E \to \mathbb{R}$ 是**费用函数**（cost function）
- $d \in \mathbb{R}^+$ 是**需求流值**（demand flow value）

### 定义 1.2 (最小费用流 / Minimum Cost Flow)

给定最小费用流网络 $N$，**最小费用流**是满足以下条件的流 $f$：

1. **容量约束**：$0 \leq f(e) \leq c(e)$ 对于所有 $e \in E$
2. **流量守恒**：对于所有 $v \in V - \{s, t\}$：
   $$\sum_{e \in \delta^+(v)} f(e) = \sum_{e \in \delta^-(v)} f(e)$$
3. **需求满足**：$|f| = d$
4. **费用最小**：在所有满足上述条件的流中，$f$ 的费用最小

**流的费用**定义为：
$$\text{cost}(f) = \sum_{e \in E} w(e) \cdot f(e)$$

---

## 2. 算法实现 / Algorithm Implementation

### 算法 2.1 (最小费用最大流算法 - 基于负环消除)

**算法描述**：

1. 初始化流 $f$ 为全零流
2. 在残差网络中寻找负费用增广路径
3. 如果找到，沿路径增加流
4. 重复步骤2-3，直到找不到增广路径或达到需求流值

**算法实现**：

```python
from typing import Dict, List, Tuple, Optional
from collections import deque
import math

class MinCostFlowNetwork:
    """
    最小费用流网络实现。
    """

    def __init__(self, vertices: List[str], edges: List[Tuple[str, str, int, int]],
                 source: str, sink: str, demand: int):
        """
        初始化最小费用流网络。

        Args:
            vertices: 顶点列表
            edges: 边列表，每个元素为 (u, v, capacity, cost)
            source: 源点
            sink: 汇点
            demand: 需求流值
        """
        self.vertices = set(vertices)
        self.source = source
        self.sink = sink
        self.demand = demand

        # 构建邻接表和容量/费用矩阵
        self.graph: Dict[str, List[str]] = {v: [] for v in vertices}
        self.capacity: Dict[Tuple[str, str], int] = {}
        self.cost: Dict[Tuple[str, str], int] = {}
        self.flow: Dict[Tuple[str, str], int] = {}

        for u, v, cap, cost_val in edges:
            self.graph[u].append(v)
            self.capacity[(u, v)] = cap
            self.cost[(u, v)] = cost_val
            self.flow[(u, v)] = 0

    def residual_capacity(self, u: str, v: str) -> int:
        """
        计算残差容量。

        Args:
            u: 起点
            v: 终点

        Returns:
            残差容量
        """
        # 正向边的残差容量
        if (u, v) in self.capacity:
            return self.capacity[(u, v)] - self.flow.get((u, v), 0)
        # 反向边的残差容量
        elif (v, u) in self.flow:
            return self.flow.get((v, u), 0)
        return 0

    def residual_cost(self, u: str, v: str) -> int:
        """
        计算残差费用。

        Args:
            u: 起点
            v: 终点

        Returns:
            残差费用
        """
        # 正向边
        if (u, v) in self.cost:
            return self.cost[(u, v)]
        # 反向边（费用取负）
        elif (v, u) in self.cost:
            return -self.cost[(v, u)]
        return 0

    def find_negative_cycle(self) -> Optional[List[str]]:
        """
        使用Bellman-Ford算法寻找负费用圈。

        Returns:
            负费用圈（如果存在），否则返回None
        """
        # 使用Bellman-Ford算法检测负环
        dist: Dict[str, float] = {v: 0 for v in self.vertices}
        parent: Dict[str, Optional[str]] = {v: None for v in self.vertices}

        # 松弛所有边 |V| 次
        for _ in range(len(self.vertices)):
            for u in self.vertices:
                for v in self.graph[u]:
                    if self.residual_capacity(u, v) > 0:
                        new_dist = dist[u] + self.residual_cost(u, v)
                        if new_dist < dist[v]:
                            dist[v] = new_dist
                            parent[v] = u

        # 检测负环
        for u in self.vertices:
            for v in self.graph[u]:
                if self.residual_capacity(u, v) > 0:
                    if dist[u] + self.residual_cost(u, v) < dist[v]:
                        # 找到负环，回溯路径
                        cycle = []
                        visited = set()
                        current = v
                        while current not in visited:
                            visited.add(current)
                            cycle.append(current)
                            current = parent[current]
                        cycle.append(current)
                        return cycle[::-1]

        return None

    def min_cost_flow(self) -> Tuple[int, int]:
        """
        计算最小费用流。

        Returns:
            (流值, 总费用)
        """
        total_flow = 0
        total_cost = 0

        # 使用增广路径方法
        while total_flow < self.demand:
            # 寻找最小费用增广路径（使用Bellman-Ford）
            path, path_flow = self.find_min_cost_path()

            if path is None:
                break

            # 沿路径增加流
            for i in range(len(path) - 1):
                u, v = path[i], path[i + 1]
                flow_increase = min(path_flow, self.residual_capacity(u, v))

                if (u, v) in self.flow:
                    self.flow[(u, v)] += flow_increase
                else:
                    self.flow[(v, u)] -= flow_increase

                total_cost += flow_increase * self.residual_cost(u, v)

            total_flow += path_flow

        return total_flow, total_cost

    def find_min_cost_path(self) -> Tuple[Optional[List[str]], int]:
        """
        使用Bellman-Ford算法寻找最小费用路径。

        Returns:
            (路径, 路径上的最小容量)
        """
        dist: Dict[str, float] = {v: math.inf for v in self.vertices}
        dist[self.source] = 0
        parent: Dict[str, Optional[str]] = {v: None for v in self.vertices}

        # 松弛 |V| - 1 次
        for _ in range(len(self.vertices) - 1):
            for u in self.vertices:
                for v in self.graph[u]:
                    if self.residual_capacity(u, v) > 0:
                        new_dist = dist[u] + self.residual_cost(u, v)
                        if new_dist < dist[v]:
                            dist[v] = new_dist
                            parent[v] = u

        # 如果无法到达汇点，返回None
        if dist[self.sink] == math.inf:
            return None, 0

        # 回溯路径
        path = []
        current = self.sink
        while current is not None:
            path.append(current)
            current = parent[current]
        path.reverse()

        # 计算路径上的最小容量
        min_capacity = math.inf
        for i in range(len(path) - 1):
            u, v = path[i], path[i + 1]
            min_capacity = min(min_capacity, self.residual_capacity(u, v))

        return path, min_capacity

# 复杂度分析
# 时间复杂度: O(V * E * F)，其中F是最大流值
# 空间复杂度: O(V + E)
```

---

## 3. 复杂度分析 / Complexity Analysis

### 定理 3.1 (最小费用流算法复杂度)

**算法 2.1** 的时间复杂度为 $O(V \cdot E \cdot F)$，其中 $F$ 是最大流值。

**证明**：

1. **路径查找**: 每次使用Bellman-Ford算法查找最小费用路径，时间复杂度 $O(V \cdot E)$
2. **增广次数**: 最多需要 $F$ 次增广（每次至少增加1单位流）
3. **总复杂度**: $O(V \cdot E \cdot F)$

### 优化方法

1. **使用Dijkstra算法**: 如果费用非负，可以使用Dijkstra算法，复杂度 $O((V + E) \log V \cdot F)$
2. **容量缩放**: 使用容量缩放技术，复杂度 $O(E^2 \log(V \cdot C) \cdot \log F)$，其中 $C$ 是最大容量
3. **成本缩放**: 使用成本缩放技术，进一步优化

---

## 4. 应用场景 / Application Scenarios

### 4.1 运输问题

**问题描述**: 从多个工厂运输货物到多个仓库，最小化运输成本。

**建模**:
- 源点：工厂
- 汇点：仓库
- 边容量：运输能力
- 边费用：单位运输成本

### 4.2 任务分配问题

**问题描述**: 将任务分配给工人，最小化总成本。

**建模**:
- 源点：任务
- 汇点：工人
- 边容量：1（每个任务分配给一个工人）
- 边费用：任务-工人匹配成本

### 4.3 网络设计问题

**问题描述**: 设计网络连接，满足流量需求，最小化建设成本。

**建模**:
- 边容量：网络容量
- 边费用：建设成本
- 需求：流量需求

---

## 🔗 **相关链接 / Related Links**

- [网络流的基本概念](01-网络流的基本概念.md)
- [最大流算法详细分析](03-最大流算法详细分析.md)
- [多源多汇流问题](04-多源多汇流问题.md)
- [图论高级理论主目录](../README.md)

---

**文档版本**: v1.0
**创建时间**: 2025年1月
**最后更新**: 2025年1月
**维护者**: GraphNetWorkCommunicate项目组
