...
使用动态规划方法。对于每个可能的袋子大小（最多 $k+1$），枚举所有可能的树分解结构。由于树宽固定，可能的分解数量是 $k$ 的指数函数，因此总时间复杂度为 $O(f(k) \cdot n)$。□
...
假设存在顶点 $v$，使得包含 $v$ 的节点集合不连通。则存在树 $T$ 中的一条边 $(t_1, t_2)$，使得 $v \in X_{t_1}$ 且 $v \in X_{t_2}$，但路径上的某个节点 $t$ 不包含 $v$。

由于 $T$ 是树，移除边 $(t_1, t_2)$ 会将 $T$ 分成两个连通分量。如果 $v$ 的包含节点不连通，则存在边 $(u, v) \in E$ 使得 $u$ 和 $v$ 不在同一个袋子中，这与树分解的定义矛盾。

因此，包含 $v$ 的节点集合必须在 $T$ 中连通。□
...
设 $C$ 是 $G$ 的最大团，$|C| = \omega(G)$。对于任意树分解 $(T, \{X_t\})$，由于 $C$ 是完全子图，$C$ 的所有顶点必须同时出现在某个袋子 $X_t$ 中（否则边不满足包含性条件）。

因此，存在 $t \in V(T)$ 使得 $C \subseteq X_t$，所以 $|X_t| \geq |C| = \omega(G)$，从而 $\text{width}(T, \{X_t\}) \geq \omega(G) - 1$。
