...
- [树分解 / Tree Decomposition](#树分解--tree-decomposition)
  - [📚 **概述 / Overview**](#-概述--overview)
  - [📑 **目录 / Table of Contents**](#-目录--table-of-contents)
  - [📐 **形式化定义 / Formal Definition**](#-形式化定义--formal-definition)
    - [定义 5.1.1 (树分解 / Tree Decomposition)](#定义-511-树分解--tree-decomposition)
    - [定义 5.1.2 (树宽 / Treewidth)](#定义-512-树宽--treewidth)
  - [🔧 **性质 / Properties**](#-性质--properties)
  - [💻 **算法实现 / Algorithm Implementation**](#-算法实现--algorithm-implementation)
    - [算法 5.1.1 (树分解构建 - 简化版)](#算法-511-树分解构建---简化版)
  - [📊 **复杂度分析 / Complexity Analysis**](#-复杂度分析--complexity-analysis)
  - [💼 **应用场景 / Application Scenarios**](#-应用场景--application-scenarios)
  - [🎯 **树分解在算法设计中的应用 / Applications of Tree Decomposition in Algorithm Design**](#-树分解在算法设计中的应用--applications-of-tree-decomposition-in-algorithm-design)
    - [动态规划在树分解中的应用](#动态规划在树分解中的应用)
      - [应用示例：独立集问题](#应用示例独立集问题)
      - [应用示例：顶点覆盖问题](#应用示例顶点覆盖问题)
    - [树宽与算法复杂度的关系](#树宽与算法复杂度的关系)
...
## 🎯 **树分解在算法设计中的应用 / Applications of Tree Decomposition in Algorithm Design**

### 动态规划在树分解中的应用

树分解的一个重要应用是在有界树宽图上设计动态规划算法。对于树宽为 $k$ 的图，许多NP难问题可以在 $O(f(k) \cdot n)$ 时间内解决，其中 $f$ 是 $k$ 的指数函数。
...
2. **动态规划**: 对树 $T$ 进行自底向上的遍历
3. **状态定义**: 对每个节点 $t$，定义 $dp[t][S]$ 表示在子树中，袋子 $X_t$ 的状态为 $S$（$S \subseteq X_t$ 是独立集）时的最大独立集大小
4. **状态转移**: 根据子节点的状态和袋子之间的关系进行转移

**时间复杂度**: $O(2^k \cdot k \cdot n)$，其中 $n$ 是图的顶点数。
...
2. **动态规划**: 对每个节点 $t$，定义 $dp[t][S]$ 表示在子树中，袋子 $X_t$ 的状态为 $S$（$S \subseteq X_t$ 是覆盖的一部分）时的最小覆盖大小
3. **状态转移**: 确保所有边都被覆盖
...
**定理**: 对于树宽为 $k$ 的图，以下问题可以在 $O(f(k) \cdot n)$ 时间内解决：

- 独立集问题：$O(2^k \cdot k \cdot n)$
- 顶点覆盖问题：$O(2^k \cdot k \cdot n)$
- 支配集问题：$O(3^k \cdot k \cdot n)$
- 着色问题：$O(k^k \cdot k \cdot n)$
- 哈密顿路径问题：$O(2^k \cdot k^2 \cdot n)$

其中 $f(k)$ 是 $k$ 的指数函数，但对于固定的 $k$，算法是线性的。
