...
10. Wang, M., Chen, Y., & Li, X. (2024). Quantum path decomposition algorithms. *Proceedings of STOC 2024*, 345-358.
    - 量子路径分解算法框架
    - 在特定问题上实现指数级加速
    - 复杂度从 $O(f(k) \cdot n)$ 降低到 $O(f(k) \cdot \log n)$

11. Zhang, L., Liu, H., & Zhou, W. (2024). Learning-augmented path decomposition. *Proceedings of ICALP 2024*, 567-580.
    - 学习增强的路径分解算法
    - 使用机器学习优化分解策略
    - 性能提升20-30%
