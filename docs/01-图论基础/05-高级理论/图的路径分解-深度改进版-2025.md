...
假设存在顶点 $v$，使得包含 $v$ 的节点集合不形成连通子路径。则存在路径 $P$ 中的节点 $t_1$ 和 $t_2$，使得 $v \in X_{t_1}$ 且 $v \in X_{t_2}$，但路径上 $t_1$ 和 $t_2$ 之间的某个节点 $t$ 不包含 $v$。

由于 $P$ 是路径，移除 $t$ 会将路径分成两个连通分量。如果 $v$ 的包含节点不连通，则存在边 $(u, v) \in E$ 使得 $u$ 和 $v$ 不在同一个袋子中，这与路径分解的定义矛盾。

因此，包含 $v$ 的节点集合必须在 $P$ 中形成连通子路径。□
