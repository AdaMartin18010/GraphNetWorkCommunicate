...
设单次运行错误概率为 $\delta$，则 $k$ 次运行中至少 $\lceil k/2 \rceil$ 次错误的概率为：

$$\mathbb{P}(\text{error}(A_k, G)) = \sum_{i=\lceil k/2 \rceil}^{k} \binom{k}{i} \delta^i (1-\delta)^{k-i}$$

当 $\delta < 1/2$ 时，使用Chernoff界：

$$\mathbb{P}(\text{error}(A_k, G)) \leq e^{-2k(1/2-\delta)^2} \leq \delta^k$$

因此，通过重复运行可以指数级降低错误概率。□
...
$$\mathbb{P}(T(A, G) > kT) \leq \frac{\mathbb{E}[T(A, G)]}{kT} = \frac{1}{k}$$

设置早期终止时间为 $kT$，则错误概率为 $1/k$。要保证错误概率 $\delta$，需要 $k = 1/\delta$，因此优化后复杂度为 $O(T \log(1/\delta))$。□
...
如果最小割的边都没有被收缩，则算法会找到最小割。在第 $i$ 次迭代中，收缩一条不在最小割中的边的概率至少为：

$$1 - \frac{k}{\text{剩余边数}} \geq 1 - \frac{k}{k(n-i+1)/2} = 1 - \frac{2}{n-i+1}$$

因此，算法找到最小割的概率至少为：

$$\prod_{i=1}^{n-2} \left(1 - \frac{2}{n-i+1}\right) = \prod_{i=1}^{n-2} \frac{n-i-1}{n-i+1} = \frac{2}{n(n-1)} \geq \frac{1}{n^2}$$
