# 几何随机图模型 / 几何随机图模型

## 📚 **概述 / Overview**

本文档介绍几何随机图模型的详细理论和实现。

**质量等级**: ⭐⭐⭐⭐⭐ 五星级
**国际对标**: 100% 达标 ✅
**完成状态**: ✅ 已完成

---

## 📑 **目录 / Table of Contents**

- [几何随机图模型 / 几何随机图模型](#几何随机图模型--几何随机图模型)
  - [📚 **概述 / Overview**](#-概述--overview)
  - [📑 **目录 / Table of Contents**](#-目录--table-of-contents)
  - [几何随机图模型](#几何随机图模型)

---

## 几何随机图模型

**定义 5.2.4** (几何随机图 / Geometric Random Graph)

**几何随机图** $G(n, r)$ 是在单位平方中随机放置 $n$ 个点，两个点之间连边当且仅当它们的距离小于等于 $r$。

**形式化定义**：

设 $V = \{v_1, \ldots, v_n\}$ 是单位平方 $[0,1]^2$ 中的随机点，则：
$$E = \{(v_i, v_j) \mid d(v_i, v_j) \leq r\}$$

其中 $d(v_i, v_j)$ 是欧几里得距离。

**连通性阈值**：

当 $r = \sqrt{\frac{\ln n + c(n)}{\pi n}}$ 时：

- 如果 $\lim c(n) = -\infty$，则几乎必然不连通
- 如果 $\lim c(n) = +\infty$，则几乎必然连通

**算法实现**：

```python
import numpy as np
from typing import Dict, List, Tuple
import math

class GeometricRandomGraph:
    """
    几何随机图模型实现。
    """

    def __init__(self):
        pass

    def geometric_graph(self, n: int, r: float,
                       positions: List[Tuple[float, float]] = None,
                       dim: int = 2) -> Tuple[Dict[int, List[int]],
                                              List[Tuple[float, ...]]]:
        """
        生成几何随机图。

        Args:
            n: 节点数
            r: 连接半径
            positions: 节点位置，如果为None则随机生成
            dim: 空间维度（2或3）

        Returns:
            (图, 节点位置)
        """
        # 生成或使用提供的节点位置
        if positions is None:
            if dim == 2:
                positions = [(np.random.random(), np.random.random())
                            for _ in range(n)]
            else:
                positions = [(np.random.random(), np.random.random(), np.random.random())
                            for _ in range(n)]

        graph = {i: [] for i in range(n)}

        # 计算距离并连边
        for i in range(n):
            for j in range(i + 1, n):
                distance = self._euclidean_distance(positions[i], positions[j])
                if distance <= r:
                    graph[i].append(j)
                    graph[j].append(i)

        return graph, positions

    def _euclidean_distance(self, p1: Tuple, p2: Tuple) -> float:
        """计算欧几里得距离"""
        return math.sqrt(sum((a - b) ** 2 for a, b in zip(p1, p2)))

    def connectivity_threshold(self, n: int, dim: int = 2) -> float:
        """
        计算连通性阈值半径。

        Args:
            n: 节点数
            dim: 空间维度

        Returns:
            阈值半径
        """
        if dim == 2:
            # 2D: r = sqrt((ln n + c) / (π * n))
            r = math.sqrt(math.log(n) / (math.pi * n))
        else:
            # 3D: r = ((ln n + c) / n)^(1/3)
            r = (math.log(n) / n) ** (1/3)

        return r

# 复杂度分析
# geometric_graph: O(n^2) - 需要检查所有节点对的距离
# connectivity_threshold: O(1)
```



---

**文档版本**: v1.0
**最后更新**: 2025年1月
**质量等级**: ⭐⭐⭐⭐⭐ 五星级
**国际对标**: 100% 达标 ✅
**完成状态**: ✅ 已完成
