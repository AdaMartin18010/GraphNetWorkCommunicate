...
- [同步动力学 / 同步动力学](#同步动力学--同步动力学)
  - [📚 **概述 / Overview**](#-概述--overview)
  - [📑 **目录 / Table of Contents**](#-目录--table-of-contents)
  - [同步动力学 / Synchronization Dynamics](#同步动力学--synchronization-dynamics)

---

## 同步动力学 / Synchronization Dynamics

**定义 5.1.4** (网络同步 / Network Synchronization)

网络上的**同步**是指网络中所有节点的状态趋于一致：
...
- $\Delta \omega = \max_i \omega_i - \min_i \omega_i$ 是频率范围
- $\lambda_2(L)$ 是拉普拉斯矩阵的第二个最小特征值（代数连通度）
...
class NetworkSynchronization:
    ...
    def kuramoto_model(self, K: float, dt: float = 0.01,
        ...
        for step in range(1, steps):
            ...
            theta += dtheta * dt
            theta = np.mod(theta, 2 * np.pi)  # 保持相位在[0, 2π]
            history[step] = theta
...
# 复杂度分析
# kuramoto_model: O(steps * n^2) - 每个时间步需要计算所有节点对的耦合
# order_parameter: O(n)
# critical_coupling: O(n^3) - 特征值计算
```
