...
**定义 5.1.5** (网络博弈 / Network Game)

**网络博弈**是网络上的博弈，每个节点的收益依赖于其邻居的策略。

**演化博弈论**：

在**演化博弈论**中，节点根据其收益更新策略。
...
- $x_i$ 是策略 $i$ 的频率
- $f_i$ 是策略 $i$ 的适应度
- $\bar{f} = \sum_j x_j f_j$ 是平均适应度

**网络上的演化博弈**：

在网络博弈中，每个节点的收益依赖于其邻居的策略：
...
class NetworkGameDynamics:
    ...
    def __init__(self, graph: Dict[int, List[int]],
        ...
        Args:
            graph: 图的邻接表表示
            payoff_matrix: 收益矩阵 [strategy_i, strategy_j] -> payoff
            strategies: 策略列表，如果为None则默认["C", "D"]（合作/背叛）
        """
        self.graph = graph
        self.payoff_matrix = payoff_matrix

        if strategies is None:
            self.strategies = ["C", "D"]  # 合作（Cooperate）、背叛（Defect）
        else:
