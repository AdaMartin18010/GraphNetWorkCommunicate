...
- [Petri网的执行语义 / Execution Semantics of Petri Nets](#petri网的执行语义--execution-semantics-of-petri-nets)
  ...
  - [1. 标识与状态 / Marking and State](#1-标识与状态--marking-and-state)
    - [1.1 标识的定义 / Definition of Marking](#11-标识的定义--definition-of-marking)
    - [1.2 初始标识 / Initial Marking](#12-初始标识--initial-marking)
    - [1.3 标识的图形表示 / Graphical Representation of Marking](#13-标识的图形表示--graphical-representation-of-marking)
  - [2. 变迁的可触发性 / Enabling of Transitions](#2-变迁的可触发性--enabling-of-transitions)
    - [2.1 可触发性的定义 / Definition of Enabling](#21-可触发性的定义--definition-of-enabling)
    - [2.2 可触发性的直观理解 / Intuitive Understanding](#22-可触发性的直观理解--intuitive-understanding)
    - [2.3 多个变迁的可触发性 / Enabling of Multiple Transitions](#23-多个变迁的可触发性--enabling-of-multiple-transitions)
  - [3. 变迁的触发 / Firing of Transitions](#3-变迁的触发--firing-of-transitions)
    - [3.1 触发规则 / Firing Rule](#31-触发规则--firing-rule)
    - [3.2 触发过程的步骤 / Steps of Firing Process](#32-触发过程的步骤--steps-of-firing-process)
    - [3.3 形式化定义 / Formal Definition](#33-形式化定义--formal-definition)
    - [3.4 触发表示法 / Firing Notation](#34-触发表示法--firing-notation)
  - [4. 状态转换示例 / State Transition Examples](#4-状态转换示例--state-transition-examples)
    - [示例 4.1：简单顺序触发 / Simple Sequential Firing](#示例-41简单顺序触发--simple-sequential-firing)
    - [示例 4.2：权重大于1的触发 / Firing with Weight \> 1](#示例-42权重大于1的触发--firing-with-weight--1)
    - [示例 4.3：一个变迁多个输出 / One Transition Multiple Outputs](#示例-43一个变迁多个输出--one-transition-multiple-outputs)
    - [示例 4.4：多个输入库所 / Multiple Input Places](#示例-44多个输入库所--multiple-input-places)
    - [示例 4.5：自循环 / Self-Loop](#示例-45自循环--self-loop)
  - [5. 并发执行 / Concurrent Execution](#5-并发执行--concurrent-execution)
    - [5.1 并发的定义 / Definition of Concurrency](#51-并发的定义--definition-of-concurrency)
    - [5.2 并发执行的示例 / Example of Concurrent Execution](#52-并发执行的示例--example-of-concurrent-execution)
    - [5.3 真并发语义 / True Concurrency Semantics](#53-真并发语义--true-concurrency-semantics)
  - [6. 冲突与冲突解决 / Conflict and Conflict Resolution](#6-冲突与冲突解决--conflict-and-conflict-resolution)
    - [6.1 冲突的定义 / Definition of Conflict](#61-冲突的定义--definition-of-conflict)
    - [6.2 冲突的示例 / Example of Conflict](#62-冲突的示例--example-of-conflict)
    - [6.3 冲突解决策略 / Conflict Resolution Strategies](#63-冲突解决策略--conflict-resolution-strategies)
      - [6.3.1 非确定性选择 / Nondeterministic Choice](#631-非确定性选择--nondeterministic-choice)
      - [6.3.2 优先级策略 / Priority Strategy](#632-优先级策略--priority-strategy)
      - [6.3.3 守卫条件 / Guard Conditions](#633-守卫条件--guard-conditions)
      - [6.3.4 概率选择 / Probabilistic Selection](#634-概率选择--probabilistic-selection)
  - [7. 执行轨迹 / Execution Traces](#7-执行轨迹--execution-traces)
    - [7.1 执行轨迹的定义 / Definition of Execution Trace](#71-执行轨迹的定义--definition-of-execution-trace)
    - [7.2 执行轨迹的表示 / Representation of Execution Traces](#72-执行轨迹的表示--representation-of-execution-traces)
      - [7.2.1 变迁序列表示 / Transition Sequence Representation](#721-变迁序列表示--transition-sequence-representation)
      - [7.2.2 状态序列表示 / State Sequence Representation](#722-状态序列表示--state-sequence-representation)
      - [7.2.3 完整轨迹表示 / Complete Trace Representation](#723-完整轨迹表示--complete-trace-representation)
    - [7.3 执行轨迹示例 / Example of Execution Trace](#73-执行轨迹示例--example-of-execution-trace)
    - [7.4 执行轨迹的分析 / Analysis of Execution Traces](#74-执行轨迹的分析--analysis-of-execution-traces)
      - [7.4.1 可达性分析 / Reachability Analysis](#741-可达性分析--reachability-analysis)
      - [7.4.2 死锁检测 / Deadlock Detection](#742-死锁检测--deadlock-detection)
      - [7.4.3 活性分析 / Liveness Analysis](#743-活性分析--liveness-analysis)
  - [8. 可达性 / Reachability](#8-可达性--reachability)
    - [8.1 可达性的定义 / Definition of Reachability](#81-可达性的定义--definition-of-reachability)
    - [8.2 可达集 / Reachability Set](#82-可达集--reachability-set)
    - [8.3 可达性图 / Reachability Graph](#83-可达性图--reachability-graph)
    - [8.4 可达性分析的应用 / Applications of Reachability Analysis](#84-可达性分析的应用--applications-of-reachability-analysis)
