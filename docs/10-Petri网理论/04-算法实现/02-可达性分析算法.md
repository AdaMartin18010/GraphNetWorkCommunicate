# 可达性分析算法 / Reachability Analysis Algorithm

## 📚 **概述 / Overview**

本文档描述Petri网可达性分析算法的实现。可达性分析是Petri网分析的核心问题之一，用于检查某个标识是否可以从初始标识到达。可达性分析在 `PetriNet` 类的 `reachability_analysis` 方法中已实现。

---

## 📑 **目录 / Table of Contents**

- [可达性分析算法 / Reachability Analysis Algorithm](#可达性分析算法--reachability-analysis-algorithm)
  - [📚 **概述 / Overview**](#-概述--overview)
  - [📑 **目录 / Table of Contents**](#-目录--table-of-contents)
  - [🔧 **功能特性 / Features**](#-功能特性--features)
  - [💻 **算法实现 / Implementation**](#-算法实现--implementation)
    - [核心算法](#核心算法)
  - [📊 **算法分析 / Algorithm Analysis**](#-算法分析--algorithm-analysis)
    - [时间复杂度](#时间复杂度)
    - [空间复杂度](#空间复杂度)
    - [复杂度说明](#复杂度说明)
  - [🔗 **相关链接 / Related Links**](#-相关链接--related-links)

---

## 🔧 **功能特性 / Features**

- 状态空间枚举
- 广度优先搜索
- 可达图构建
- 可达性判定
- 死锁检测

---

## 💻 **算法实现 / Implementation**

可达性分析算法已在 `PetriNet` 类的 `reachability_analysis` 方法中实现。该方法使用广度优先搜索（BFS）遍历所有可达标识。

### 核心算法

```python
def reachability_analysis(self, max_depth: int = 100) -> Set[Tuple]:
    """
    进行可达性分析。

    Args:
        max_depth: 最大搜索深度

    Returns:
        可达标识集
    """
    initial_marking_tuple = tuple(sorted(self.marking.items()))
    self.reachability_set = {initial_marking_tuple}
    queue = [(initial_marking_tuple, 0)]

    while queue:
        marking_tuple, depth = queue.pop(0)
        if depth >= max_depth:
            continue

        # 恢复标识
        self.marking = dict(marking_tuple)

        # 尝试触发所有变迁
        for transition in self.transitions:
            if self.is_enabled(transition):
                old_marking = dict(self.marking)
                self.fire(transition)
                new_marking_tuple = tuple(sorted(self.marking.items()))

                # 记录可达性图
                self.reachability_graph[marking_tuple].append((transition, new_marking_tuple))

                # 如果新标识未访问过，加入队列
                if new_marking_tuple not in self.reachability_set:
                    self.reachability_set.add(new_marking_tuple)
                    queue.append((new_marking_tuple, depth + 1))

                # 恢复标识
                self.marking = old_marking

    # 恢复初始标识
    self.marking = dict(initial_marking_tuple)
    return self.reachability_set
```

---

## 📊 **算法分析 / Algorithm Analysis**

### 时间复杂度

- **可达性分析**: $O(|R| \cdot |T| \cdot (|P| + |T|))$
  - 其中 $|R|$ 是可达标识数
  - $|T|$ 是变迁数
  - $|P|$ 是库所数

### 空间复杂度

- $O(|R| \cdot |P| + |R| \cdot |T|)$ - 用于存储可达集和可达性图

### 复杂度说明

可达性问题是 **PSPACE-complete** 的，这意味着对于大规模Petri网，完整的状态空间搜索可能非常耗时。实际应用中通常使用：

- 状态空间约简技术
- 符号状态表示
- 启发式搜索方法

---

## 🔗 **相关链接 / Related Links**

- [Petri网理论主目录](../../README.md)
- [算法实现目录](../README.md)
- [可达性分析](../../03-分析方法/01-可达性分析.md)
- [Petri网模拟器](01-Petri网模拟器.md)

---

**文档版本**: v1.0
**创建时间**: 2025年1月
**状态**: ✅ **已完成**
