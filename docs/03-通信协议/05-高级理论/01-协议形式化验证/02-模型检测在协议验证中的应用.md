...
#### 步骤1: 穷举搜索 / Step 1: Exhaustive Search

**引理 2.1.1.1**：模型检测算法通过穷举搜索访问所有可达状态。

**证明**：

**搜索算法**：

- 模型检测算法使用BFS或DFS从初始状态开始遍历
- 算法访问所有从初始状态可达的状态
- 由于状态空间有限，算法会终止并访问所有可达状态

**可达性保证**：

- 如果状态 $s$ 可达，则存在从初始状态到 $s$ 的路径
- BFS/DFS会沿着这条路径访问 $s$
- 因此所有可达状态都会被访问

**结论**：模型检测算法访问所有可达状态。$\square$

#### 步骤2: 性质检查 / Step 2: Property Checking

**引理 2.1.1.2**：对于每个访问的状态，算法检查其是否满足性质。

**证明**：

**检查过程**：

- 对于每个访问的状态 $s$，算法检查性质 $\phi$ 是否在 $s$ 上成立
- 如果 $\phi(s)$ 不成立，则 $s$ 是违反性质的状态
- 算法记录所有违反性质的状态

**检查完备性**：

- 由于算法访问所有可达状态，且对每个状态都进行检查
- 因此所有违反性质的状态都会被检测到

**结论**：算法检查所有访问的状态是否满足性质。$\square$

#### 步骤3: 检测完备性 / Step 3: Detection Completeness

**定理证明**：

由引理2.1.1.1和引理2.1.1.2：

- 算法访问所有可达状态（引理2.1.1.1）
- 算法检查所有访问的状态（引理2.1.1.2）
- 因此所有违反性质的状态都会被检测到
...
#### 步骤1: 形式化语义基础 / Step 1: Formal Semantics Foundation

**引理 2.1.2.1**：模型检测算法基于形式化语义，严格按照时序逻辑的语义进行验证。

**证明**：

**语义定义**：

- 时序逻辑（CTL/LTL）有严格的形式化语义
- 模型检测算法按照这些语义定义进行验证
- 每个状态和路径的验证都遵循语义规则
...
- 算法的每一步都对应语义定义中的某个操作
- 例如，CTL公式 $\mathbf{AG}\phi$ 的验证对应"所有路径上所有状态都满足$\phi$"
- 算法的实现严格遵循这个语义

**结论**：算法基于形式化语义进行验证。$\square$

#### 步骤2: 满足性判断正确性 / Step 2: Satisfaction Judgment Correctness

**引理 2.1.2.2**：如果算法返回"满足"，则模型确实满足性质。
...
- 设算法返回"满足"，则算法验证了所有可达状态都满足性质 $\phi$
- 形式化：$\forall s \in R: s \models \phi$，其中 $R$ 是可达状态集合

**语义满足关系**：

- 由语义定义，模型满足性质：$\mathcal{M} \models \phi \iff \forall \pi \in \text{Paths}(\mathcal{M}): \pi \models \phi$
- 其中 $\text{Paths}(\mathcal{M})$ 是所有从初始状态开始的路径集合
- 对于路径 $\pi = s_0, s_1, s_2, \ldots$，路径满足性质：$\pi \models \phi \iff \forall i \geq 0: s_i \models \phi$

**可达状态与路径的关系**：

- 对于任意路径 $\pi = s_0, s_1, s_2, \ldots$，路径上的所有状态 $s_i$ 都是可达的：$\forall i \geq 0: s_i \in R$
- 因此：$\forall \pi \in \text{Paths}(\mathcal{M}): \forall i \geq 0: s_i \in R \land s_i \models \phi$
- 因此：$\forall \pi \in \text{Paths}(\mathcal{M}): \pi \models \phi$
- 因此：$\mathcal{M} \models \phi$

**结论**：如果算法返回"满足"，则模型确实满足性质。$\square$

#### 步骤3: 不满足性判断正确性 / Step 3: Non-satisfaction Judgment Correctness

**引理 2.1.2.3**：如果算法返回"不满足"，则模型确实不满足性质。
...
- 设算法返回"不满足"，则算法找到了可达状态 $s \in R$ 不满足性质 $\phi$
- 形式化：$\exists s \in R: s \not\models \phi$

**反例路径构造**：

- 由于 $s$ 是可达的，存在路径 $\pi = s_0, s_1, \ldots, s_k = s$ 从初始状态 $s_0$ 到 $s$
- 由于 $s \not\models \phi$，路径 $\pi$ 不满足性质：$\pi \not\models \phi$
- 因此存在路径 $\pi \in \text{Paths}(\mathcal{M})$ 使得 $\pi \not\models \phi$

**语义不满足关系**：

- 由语义定义，模型不满足性质：$\mathcal{M} \not\models \phi \iff \exists \pi \in \text{Paths}(\mathcal{M}): \pi \not\models \phi$
- 由于存在路径 $\pi$ 不满足性质，因此 $\mathcal{M} \not\models \phi$

**结论**：如果算法返回"不满足"，则模型确实不满足性质。$\square$

#### 步骤4: 检测正确性 / Step 4: Detection Correctness
...
#### 步骤1: 可达性计算复杂度 / Step 1: Reachability Computation Complexity

**引理 2.2.2.1**：计算可达状态集合的时间复杂度为 $O(|S| \cdot |\Sigma|)$。
...
- 使用BFS或DFS从初始状态集合 $S_0$ 开始遍历
- 设访问标记数组 $visited[s]$ 标记状态 $s$ 是否已访问
- 对于每个访问的状态 $s$，检查所有可能的转移 $(s, \sigma, s') \in \Delta$
...
- 对于每个状态 $s$，转移数量为 $|\{(s, \sigma, s') \in \Delta\}| \leq |\Sigma|$（每个符号最多一个转移）
- 总转移检查数：$\sum_{s \in S} |\{(s, \sigma, s') \in \Delta\}| \leq |S| \cdot |\Sigma|$
- 因此转移检查数为 $O(|S| \cdot |\Sigma|)$

**复杂度计算**：

- 状态访问数：$O(|S|)$
- 转移检查数：$O(|S| \cdot |\Sigma|)$
- 总时间复杂度：$O(|S| \cdot |\Sigma|)$（由转移检查数主导）

**结论**：计算可达状态集合的时间复杂度为 $O(|S| \cdot |\Sigma|)$。$\square$

#### 步骤2: 性质检查复杂度 / Step 2: Property Checking Complexity

**引理 2.2.2.2**：检查所有可达状态是否满足性质的时间复杂度为 $O(|S|)$。
...
- 对于每个可达状态 $s \in R$（$R$ 是可达状态集合），检查性质 $\phi$ 是否在 $s$ 上成立
- 性质检查：$s \models \phi$（状态 $s$ 满足性质 $\phi$）
- 状态检查的复杂度取决于性质的复杂度：
  - 原子命题：$O(1)$
  - 布尔组合：$O(|\phi|)$（其中 $|\phi|$ 是公式大小）
  - CTL公式：$O(|\phi|)$（递归检查）

**状态检查数分析**：

- 可达状态集合 $R \subseteq S$，因此 $|R| \leq |S|$
- 最多检查 $|S|$ 个状态

**复杂度计算**：

- 状态检查数：$O(|S|)$
- 每个状态检查：$O(|\phi|)$（其中 $|\phi|$ 是公式大小）
- 总时间复杂度：$O(|S| \cdot |\phi|)$
- 如果 $|\phi| = O(1)$（性质大小是常数），则总时间复杂度为 $O(|S|)$

**结论**：检查所有可达状态是否满足性质的时间复杂度为 $O(|S|)$。$\square$
...
- 可达性计算：$O(|S| \cdot |\Sigma|)$
- 性质检查：$O(|S|)$
- 总时间复杂度：$O(|S| \cdot |\Sigma|)$（主导项）

**结论**：模型检测算法的时间复杂度为 $O(|S| \cdot |\Sigma|)$。$\square$

#### 步骤4: 空间复杂度 / Step 4: Space Complexity

**引理 2.2.2.3**：模型检测算法的空间复杂度为 $O(|S|)$。
...
- 存储访问标记数组 $visited[s]$：需要 $|S|$ 个布尔值
- 空间复杂度：$O(|S|)$

**队列/栈存储**：

- BFS使用队列，DFS使用栈
- 最坏情况：所有状态都在队列/栈中（当状态空间是链状结构时）
- 空间复杂度：$O(|S|)$

**可达状态集合存储**：

- 存储可达状态集合 $R$：需要 $|R| \leq |S|$ 个状态引用
- 空间复杂度：$O(|S|)$
...
- 队列/栈：$O(|S|)$
- 可达状态集合：$O(|S|)$
- 总空间复杂度：$O(|S|)$（所有项都是 $O(|S|)$）

**结论**：模型检测算法的空间复杂度为 $O(|S|)$。$\square$

#### 步骤5: 定理结论 / Step 5: Theorem Conclusion

**定理证明**：

由引理2.2.2.1、2.2.2.2和2.2.2.3：
...
- **协议形式化建模**：参见：[协议的形式化建模方法](01-协议的形式化建模方法.md) - 模型检测需要协议的形式化模型
- **协议正确性规范**：参见：[协议正确性的形式化规范](04-协议正确性的形式化规范.md) - 模型检测验证协议是否满足规范
- **定理证明**：参见：[定理证明在协议验证中的应用](03-定理证明在协议验证中的应用.md) - 模型检测和定理证明是互补的验证方法

### 5.5.2 协议设计相关理论

**相关理论**：

- **协议形式化验证**：参见：[协议形式化验证](../协议形式化验证-深度改进版-2025.md) - 模型检测是形式化验证的核心方法
- **协议安全性分析**：参见：[协议安全性分析](../03-协议安全性分析-深度改进版-2025.md) - 使用模型检测分析协议安全性
- **协议性能分析**：参见：[协议性能分析](../协议性能分析-深度改进版-2025.md) - 模型检测可以分析协议性能性质

### 5.5.3 形式化证明相关理论

**跨模块引用**：

- **模型检测**：参见：[模型检测](../../08-形式化证明/03-模型检测.md) - 模型检测的理论基础
- **时序逻辑**：参见：[时序逻辑](../../08-形式化证明/02-时序逻辑.md) - 模型检测使用时序逻辑描述性质
- **形式化语义**：参见：[形式化语义](../../08-形式化证明/04-形式化语义.md) - 模型检测的形式化语义

### 5.5.4 图论相关理论

**跨模块引用**：

- **图的算法**：参见：[图的算法](../../01-图论基础/03-图的算法.md) - 状态空间搜索算法
- **图的连通性**：参见：[图的连通性](../../01-图论基础/02-连通性.md) - 可达性分析

### 5.5.5 理论关系总结

**关系类型**：

- **应用关系**：模型检测应用于协议验证、安全性分析、性能分析
- **依赖关系**：模型检测依赖于形式化建模、时序逻辑、图算法
- **互补关系**：模型检测与定理证明是互补的验证方法
- **扩展关系**：模型检测可以扩展到实时系统、概率系统等领域
